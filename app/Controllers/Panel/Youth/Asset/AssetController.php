...
class AssetController extends BaseController
{
	...
	public function get()
	{
		if ($this->libIonix->Decode($this->request->getGet('scope')) == 'category') {
			if ($this->request->getGet('format') == 'JSON') {
				return requestOutput(200, NULL, $this->libIonix->builderQuery('youth_asset_categorys')->where(['asset_category_id' => $this->libIonix->Decode($this->request->getGet('id'))])->get()->getRow());
			}
		}
	...
	public function store()
	{
		if ($this->libIonix->Decode($this->request->getGet('scope')) == 'category') {
			if ($this->request->getGet('id') == 'add') {
				...
				return $this->updateCategory($this->libIonix->builderQuery('youth_asset_categorys')->where(['asset_category_id' => $this->libIonix->Decode($this->request->getGet('id'))])->get()->getRow());
			}
		}
	...
	private function addAsset()
	{
		...
		// dd($requestAsset);
		$query = (object) [
			'insert'		=> $this->libIonix->insertQuery('youth_assets', $requestAsset),
		];
		...
		foreach ($this->modUser->fetchData(['role_access >=' => $this->configIonix->roleController, 'active' => true], false, 'DESC', false)->get()->getResult() as $row) {
			$requestNotification	= [
				...
				'notification_slug'			=> 'youth_assets/'.$this->libIonix->Encode($query->insert).'/manage',
	...
	private function addVerify(object $assetData)
	{
		...
		$requestNotification 		= [
			...
			'notification_slug'			=> 'youth_assets/'.$this->libIonix->Encode($assetData->asset_id).'/manage',
	...
	private function addImage(object $assetData)
	{
		...
		if ($this->libIonix->builderQuery('youth_asset_images')->where(['asset_id' => $assetData->asset_id])->countAllResults() >= 3) {
		...
		$output = [
			'insert'		=> $this->libIonix->insertQuery('youth_asset_images', $request),
	...
	private function updateAsset(object $assetData)
	{
		...
		$output = [
			'update'						=> $this->libIonix->updateQuery('youth_assets', ['asset_id' => $assetData->asset_id], $requestAsset),
			'pushNotification'	=> $this->libIonix->pushNotification(),
			'flash'   					=> $this->session->setFlashdata('alertToastr', [
															 'type'			=> 'success',
															 'header'		=> '202 Accepted',
