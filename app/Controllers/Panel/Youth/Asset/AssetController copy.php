...
class AssetController extends BaseController
{
	...
	public function index()
	{
		...
		return view('panels/youths/assets/assets', $this->libIonix->appInit($data));
	}
	...
	public function detail()
	{
		...
		if ($this->modAsset->fetchData($parameters)->countAllResults() == true) {
			...
			return view('panels/youths/assets/assets-detail', $this->libIonix->appInit($data));
		}
			...
			$subArray[] = '<div class="text-center">
												<div class="dropdown dropstart">
														...
														<div class="dropdown-menu">
																<a class="dropdown-item" href="'.panel_url('youth_assets/'.$this->libIonix->Encode($row->asset_id).'/manage').'"><i class="mdi mdi-vector-link font-size-16 align-middle text-primary me-1"></i> Rincian & Kelola</a>
	...
	private function addCategory()
	{
		...
		if ($this->libIonix->builderQuery('youth_asset_categorys')->where(['asset_category_name' => $request['asset_category_name']])->countAllResults() > 0) {
		...
		$output = [
			'insert' 	=> $this->libIonix->insertQuery('youth_asset_categorys', $request),
	...
	private function updateCategory(object $categoryData)
	{
		...
		$output = [
			'update'	=> $this->libIonix->updateQuery('youth_asset_categorys', ['asset_category_id' => $categoryData->asset_category_id], $request),
			'flash'   => $this->session->setFlashdata('alertToastr', [
											'type'		=> 'success',
											'header'	=> '202 Accepted',
	...
	private function addAsset()
	{
		$requestAsset = [
			...
			'asset_district_id'			=> $this->request->getPost('district'),
			'asset_type'						=> $this->request->getPost('type'),
			'asset_ownership'				=> $this->request->getPost('ownership'),
			...
			'asset_approve'					=> isStakeholder() == false ? 3 : 2,
			'asset_approve_by'			=> isStakeholder() == false ? $this->libIonix->getUserData(NULL, 'object')->user_id : NULL,
			'asset_created_by'			=> $this->libIonix->getUserData(NULL, 'object')->user_id,
		];

		$query = (object) [
			'insert'		=> $this->libIonix->insertQuery('assets', $requestAsset),
		];
		...
		foreach ($this->modUser->fetchData(['role_access >=' => $this->configIonix->roleController, 'active' => true], false, 'DESC', false)->get()->getResult() as $row) {
			$requestNotification	= [
				...
				'notification_slug'			=> $requestAsset['asset_ownership'].'_assets/'.$this->libIonix->Encode($query->insert).'/manage',
	...
	private function addVerify(object $assetData)
	{
		...
		$requestNotification 		= [
			...
			'notification_slug'			=> $assetData->asset_ownership.'_assets/'.$this->libIonix->Encode($assetData->asset_id).'/manage',
	...
	private function addImage(object $assetData)
	{
		...
		if ($this->libIonix->builderQuery('asset_images')->where(['asset_id' => $assetData->asset_id])->countAllResults() >= 3) {
		...
		$output = [
			'insert'		=> $this->libIonix->insertQuery('asset_images', $request),
	...
	private function updateAsset(object $assetData)
	{
		...
		foreach ($this->modUser->fetchData(['role_access >=' => $this->configIonix->roleController, 'active' => true], false, 'DESC', false)->get()->getResult() as $row) {
			$requestNotification	= [
				...
				'notification_slug'			=> $requestAsset['asset_ownership'].'_assets/'.$this->libIonix->Encode($assetData->asset_id).'/manage',
		...
		$output = [
			'update'						=> $this->libIonix->updateQuery('assets', ['asset_id' => $assetData->asset_id], $requestAsset),
			'pushNotification'	=> $this->libIonix->pushNotification(),
			'flash'   					=> $this->session->setFlashdata('alertToastr', [
															 'type'			=> 'success',
															 'header'		=> '202 Accepted',
	...
	private function uploadAttachment(object $assetData)
	{
		...
		$output = [
			'update'		=> $this->libIonix->updateQuery('youth_assets', ['asset_id' => $assetData->asset_id], ['file_id' => $upload->insert]),
		];
	...
	private function deleteAsset(object $assetData)
	{
		$output = [
			...
			'delete' 	=> $this->libIonix->deleteQuery('youth_assets', ['asset_id' => $assetData->asset_id]),
		];
 ...
 * Location: ./app/Controllers/Panel/Youth/Asset/AssetController.php
 * -----------------------------------------------------------------------
 */
