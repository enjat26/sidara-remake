...
class DistrictModel extends Model
{
  ...
  public function fetchData(array $where = NULL, bool $limit = false, string $order = 'DESC')
  {
    $query = $this->table($this->table)
                  ...
                  ->join('provinces', 'provinces.province_id = '.$this->table.'.province_id')
  ...
  private function filterData($query)
  {
    ...
    if(!empty($this->construct()->request->getVar('columns')[1]['search']['value']))
    {
       $i = 0;
       foreach ($this->allowedSearch as $item)
       {
         if($this->construct()->request->getVar('columns')[1]['search']['value']) {
           if ($i === 0) {
             $query->groupStart();
             $query->like($item, $this->construct()->request->getVar('columns')[1]['search']['value']);
           } else {
             $query->orLike($item, $this->construct()->request->getVar('columns')[1]['search']['value']);
           }
           if(count($this->allowedSearch) - 1 == $i)
           $query->groupEnd();
         }
...
/**
 * -----------------------------------------------------------------------
 * Filename: Area/DistrictModel.php
 * Location: ./app/Models/Area/DistrictModel.php
 * -----------------------------------------------------------------------
 */
