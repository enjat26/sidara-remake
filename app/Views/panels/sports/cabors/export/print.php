              ...
              <table class="table styled-table align-middle w-100">
                  <thead>
                      <tr>
                          ...
                          <th scope="col" class="text-center align-middle">Nama Cabang Olahraga/Kode</th>
                          <th scope="col" class="text-center align-middle">Ketua Cabang Olahraga</th>
                  ...
                  <tbody>
                      <?php $i = 1;?>
                      <?php foreach ($data['modCabor']->fetchData($data['parameters'])->get()->getResult() as $row) : ?>
                          <tr>
                              <th scope="row" class="text-center"><strong><?= $i++; ?>.</strong></th>
                              <td>
                                  <h6 class="text-truncate mb-0"><?= $row->sport_cabor_name;?></h6>
                                  <p class="text-muted mb-0"><?= strtoupper($row->sport_cabor_code);?></p>
                              </td>
                              <td class="text-center"><?= $row->sport_cabor_leader;?></td>
                              <td class="text-center"><?= $row->district_type.' '.$row->district_name.', '.$row->province_name;?></td>
                              <td class="text-center"><?= $row->sport_cabor_year_start.' - '.$row->sport_cabor_year_end;?></td>
                              <td class="text-center"><?= $row->sport_cabor_file_id ? 'Dokumen terlampir' : '-' ;?></td>
                              <td>
                                  <h6 class="text-truncate mb-0"><?= $libIonix->getUserData(['users.user_id' => $row->sport_cabor_created_by], 'object')->name;?></h6>
                                  <p class="text-muted mb-0"><?= $libIonix->getUserData(['users.user_id' => $row->sport_cabor_created_by], 'object')->role_name;?></p>
                              </td>
                          </tr>
                      <?php endforeach; ?>
                      <tr>
                          <th scope="row" colspan="6" class="text-end">Jumlah Cabang Olahraga Olahraga</th>
                          <td class="text-center"><strong><?= $data['modCabor']->fetchData($data['parameters'])->countAllResults();?></strong></td>
                      </tr>
                  </tbody>
              </table>
            </tr>
