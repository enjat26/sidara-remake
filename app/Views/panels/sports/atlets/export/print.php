            ...
            <table class="table styled-table align-middle w-100">
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="text-center align-middle" colspan="3">TTL</th>
                        <th scope="col" class="text-center align-middle" rowspan="2">Agama</th>
                        <th scope="col" class="text-center align-middle" rowspan="2">Asal Daerah</th>
                        <th scope="col" class="text-center align-middle" rowspan="2">Status</th>
                        <th scope="col" class="text-center align-middle" rowspan="2">Binaan</th>
                ...
                <tbody>
                    <?php $i = 1;?>
                    <?php foreach ($data['modAtlet']->fetchData($data['parameters'])->get()->getResult() as $row) : ?>
                        <tr>
                            <th scope="row" class="text-center"><strong><?= $i++; ?>.</strong></th>
                            <td>
                                <h6 class="text-truncate mb-0"><?= $row->sport_atlet_name;?></h6>
                                <p class="text-muted mb-0"><?= $row->sport_atlet_code;?></p>
                            </td>
                            <td class="text-center"><?= parseGender($row->sport_atlet_gender)?></td>
                            <td>
                                <div class="media">
                            ...
                            <td class="text-center"><?= $row->sport_atlet_dob ? parseAge($row->sport_atlet_dob)->years.' Tahun' : '-' ;?></td>
                            <td class="text-center"><?= $row->sport_atlet_pob ? $row->sport_atlet_pob : '-' ;?></td>
                            <td class="text-center"><?= $row->sport_atlet_dob ? parseDate($row->sport_atlet_dob) : '-' ;?></td>
                            <td class="text-center"><?= $row->sport_atlet_religion ? $row->sport_atlet_religion : '-';?></td>
                            <td class="text-center"><?= $row->district_type.' '.$row->district_name.', '.$row->province_name;?></td>
                            <td class="text-center"><?= $row->sport_atlet_status == true ? 'Aktif' : 'Tidak Aktif' ;?></td>
                            <td class="text-center"><?= $row->sport_atlet_ownership;?></td>
                            <td>
                                <h6 class="text-truncate mb-0"><?= $libIonix->getUserData(['users.user_id' => $row->sport_atlet_created_by], 'object')->name;?></h6>
                                <p class="text-muted mb-0"><?= $libIonix->getUserData(['users.user_id' => $row->sport_atlet_created_by], 'object')->role_name;?></p>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    <tr>
                        <th scope="row" colspan="11" class="text-end">Jumlah Atlet</th>
                        <td class="text-center"><strong><?= $data['modAtlet']->fetchData($data['parameters'])->countAllResults();?></strong></td>
                    </tr>
                </tbody>
            </table>
        </div>
