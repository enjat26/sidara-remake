...
<?= $this->section('meta');?>
    <meta name="scope" content="<?= $libIonix->Encode('province');?>">
<?= $this->endSection();?>
...
<?= $this->section('content');?>
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Provinsi</h4>
    ...
    <div class="row justify-content-center">
      <div class="col-md-6 col-xl-3">
          <div class="card">
              <div class="card-body">
                  <h4 class="card-title mb-3">Data Provinsi</h4>
                  <p class="card-title-desc text-justify">
                      Provinsi adalah nama sebuah pembagian <strong>Wilayah Administratif</strong> di bawah wilayah <strong>Nasional/Negara</strong>.
                      Sama seperti halnya Negara, Provinsi sebagai identitas yang dibutuhkan sebagai <strong>Atribut</strong> dalam informasi seperti Instansi/Badan Usaha, Pengguna, dan lainnya.
                  </p>

                  <p class="mb-4">Beberapa aksi yang dapat Anda lakukan.</p>

                  <div class="button-items mb-4">
                      <button type="button" class="btn btn-success waves-effect waves-light" key="rfs-province"><i class="mdi mdi-refresh me-1"></i>Segarkan</button>
                      <button type="button" class="btn btn-light waves-effect waves-light" data-bs-toggle="modal" data-bs-target="#modal-province" key="add-province"><i class="mdi mdi-plus me-1"></i>Tambah provinsi baru</button>
                  </div>
              </div>
          </div>
      </div>
          ...
          <div class="card-body">
              <div class="card border border-success">
                  <div class="card-header bg-transparent border-success">
                      ...
                      <p class="card-text">Anda dapat memfilter data <strong>Provinsi</strong> berdasarkan</p>
                  </div>
                  <div class="card-body">
                      <form id="form-export" class="needs-validation" action="<?= panel_url('provinces/export/print');?>" target="_blank" method="GET" novalidate>
                          <div class="row justify-content-end">
                              <div class="col-sm-4">
                                  <div class="card border">
                                      <div class="card-body">
                                          <h5>Negara</h5>
                                          <p class="text-muted mb-0"><strong>Provinsi</strong> akan dikelompokan sesuai <strong>Negara</strong></p>
                                      </div>
                                      <div class="card-footer bg-transparent border-top text-center">
                                          <div class="form-group">
                                              <select class="form-control select2" name="filter-country" aria-hidden="true" data-placeholder="Pilih negara...">
                                                  <option></option>
                          ...
                          <div class="row">
                              <div class="col-md-6 col-xl-7 my-auto">
                                  <p class="text-muted mb-0">Ekspor Data akan ditampilkan berdasarkan filter yang dipilih.</p>
                              </div>
              ...
              <table id="dt_provinces" class="table table-striped table-borderless align-middle w-100 mt-2">
                  <thead>
                      <tr>
    ...
    <div class="modal fade" id="modal-province" role="dialog" aria-labelledby="modal-title" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="card-description text-center">
                        Sesuaikan informasi <strong>Provinsi</strong> pada bidang-bidang dibawah ini.
                        Jika ada bidang yang bertanda <code>*</code>, maka bidang tersebut wajib diisi.
                    </p>

                    <?= customFormOpen('province');?>
                        <h5 class="text-center my-md-3">Wilayah</h5>

                        <div class="form-group position-relative">
                            <label for="country">Negara<code>*</code></label>
                            <select class="form-control select2" name="country" aria-hidden="true" data-placeholder="Pilih negara..." required>
                                <option></option>
                                <?php foreach ($data['modCountry']->fetchData(NULL, false, 'CUSTOM')->orderBy('country_name', 'ASC')->get()->getResult() as $row): ?>
                                    <option value="<?= $row->country_id;?>"><?= ucwords($row->country_name);?> (<?= strtoupper($row->country_iso3);?>)</option>
                                <?php endforeach; ?>
                        ...
                        <div class="form-group position-relative">
                            <label for="name">Nama Provinsi<code>*</code></label>
                            <input type="text" name="name" class="form-control" placeholder="Masukan nama provinsi" required>
                        </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-<?= $configIonix->colorPrimary;?> waves-effect waves-light" onclick="$('#form-province').submit();">Button</button>
                </div>
            </div>
            <!-- end modal-content -->
        </div>
...
<?= $this->section('javascript');?>
    ...
    <?= script_tag($configIonix->assetsFolder['local'].'js/panel/area/provinces.init.js');?>
<?= $this->endSection();?>
