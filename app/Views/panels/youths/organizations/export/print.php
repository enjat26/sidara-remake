                  ...
                  <tbody>
                      ...
                      <?php foreach ($data['modOrganization']->fetchData($data['parameters'])->get()->getResult() as $row) : ?>
                          <tr>
                              ...
                              <td>
                                  <h6 class="text-truncate mb-0"><?= $row->youth_organization_name;?></h6>
                                  <p class="text-muted mb-0"><?= strtoupper($row->youth_organization_code);?></p>
                              </td>
                              <td class="text-center"><?= $row->youth_organization_leader;?></td>
                              <td class="text-center"><?= $row->district_type.' '.$row->district_name.', '.$row->province_name;?></td>
                              <td class="text-center"><?= $row->youth_organization_year_start.' - '.$row->youth_organization_year_end;?></td>
                              <td class="text-center"><?= $row->youth_organization_file_id ? 'Dokumen terlampir' : '-' ;?></td>
                              <td>
                                  <h6 class="text-truncate mb-0"><?= $libIonix->getUserData(['users.user_id' => $row->youth_organization_created_by], 'object')->name;?></h6>
                                  <p class="text-muted mb-0"><?= $libIonix->getUserData(['users.user_id' => $row->youth_organization_created_by], 'object')->role_name;?></p>
                              </td>
                          </tr>
                      <?php endforeach; ?>
                      <tr>
                          <th scope="row" colspan="6" class="text-end">Jumlah Organisasi Pemuda</th>
