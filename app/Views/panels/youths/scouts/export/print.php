            ...
            <table class="table styled-table align-middle w-100">
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="text-center align-middle">Penyelenggara</th>
                ...
                <tbody>
                    <?php $i = 1;?>
                    <?php foreach ($data['modScout']->fetchData($data['parameters'])->get()->getResult() as $row): ?>
                        <tr>
                            <th scope="row" class="text-center"><strong><?= $i++; ?>.</strong></th>
                            <td>
                              <h6 class="text-truncate mb-0"><?= $row->scout_championship_name;?></h6>
                              <p class="text-muted mb-0"><?= $row->scout_championship_level;?></p>
                            </td>
                            <td class="text-center"><?= $row->scout_championship_organizer;?></td>
                            <td>
                                <h6 class="text-truncate mb-0"><?= $row->scout_participant_name;?></h6>
                                <p class="text-muted mb-0"><?= parseGender($row->scout_participant_gender);?></p>
                            </td>
                            <td class="text-center">
                                <?php if ($row->scout_participant_address): ?>
                                        <?= $row->scout_participant_address;?>
                                    <?php else: ?>
                                        -
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="text-center">
                                <?php if ($row->scout_championship_result): ?>
                                        <?= $row->scout_championship_result;?>
                                    <?php else: ?>
                                        -
                                <?php endif; ?>
                            </td>
                            ...
                            <td class="text-center">
                                <?= $libIonix->getUserData(['users.user_id' => $row->scout_created_by], 'object')->name;?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                        <tr>
                            <th scope="row" colspan="8" class="text-end">Jumlah Prestasi Pramuka</th>
                            <td class="text-center"><strong><?= $data['modScout']->fetchData($data['parameters'])->countAllResults();?></strong></td>
                        </tr>
                </tbody>
            </table>
        </div>
