...
function initButton() {
  ...
  $('[key="add-cabor"]').on("click", function () {
    ...
    $('#modal-cabor .modal-title').text('Tambah Data Cabang Olahraga'),
...
function fetchCabor() {
  var dataTable = $('#dt_cabors').DataTable({
    ...
    "order": [],
    "ajax": {
      url: $('meta[name=site-url]').attr("content")+"sport_cabors/list?scope="+$('meta[name=scope]').attr("content")+"&format=DT",
      type: "POST",
      processData: true,
    },
    "columnDefs": [{
...
$("#form-cabor").validate({
  submitHandler: function(form) {
    $.ajax({
      url:  $('meta[name=site-url]').attr("content")+'sport_cabors/store?scope='+scope+'&id='+value,
...
function putCabor(method, id) {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      $('[name="year_start"]').val(response.data.sport_cabor_year_start).trigger('change'),
      $('[name="year_end"]').val(response.data.sport_cabor_year_end).trigger('change'),
      $('#form-cabor [name="name"]').val(response.data.sport_cabor_name),
      $('[name="leader"]').val(response.data.sport_cabor_leader),
      $('[name="address"]').val(response.data.sport_cabor_address),
      $('[name="province"]').val(response.data.province_id).trigger('change');

      if (response.data.sport_cabor_file_id) {
...
function deleteMethod(redirect, scope, id) {
  Swal.fire({
    ...
    if(result.value) {
      $.ajax({
          url: $('meta[name=site-url]').attr("content")+'sport_cabors/delete?scope='+scope+'&id='+id,
          type: "DELETE",
          success: function(response){
            if (redirect == true) {
              location.reload();
