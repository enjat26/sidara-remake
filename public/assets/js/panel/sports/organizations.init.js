...
function initButton() {
  ...
  $('[key="add-organization"]').on("click", function () {
    ...
    $('#modal-organization .modal-title').text('Tambah Data Organisasi Olahraga'),
...
function fetchOrganization() {
  var dataTable = $('#dt_organizations').DataTable({
    ...
    "order": [],
    "ajax": {
      url: $('meta[name=site-url]').attr("content")+"sport_organizations/list?scope="+$('meta[name=scope]').attr("content")+"&format=DT",
      type: "POST",
      processData: true,
    },
    "columnDefs": [{
...
$("#form-organization").validate({
  submitHandler: function(form) {
    $.ajax({
      url:  $('meta[name=site-url]').attr("content")+'sport_organizations/store?scope='+scope+'&id='+value,
...
function putOrganization(method, id) {
  ...
  $.ajax({
    url:  $('meta[name=site-url]').attr("content")+'sport_organizations/get',
    data: "scope="+scope+"&format=JSON&id="+id,
    type: "GET",
    success: function(response) {
      $('[name="code"]').val(response.data.sport_organization_code),
      $('[name="year_start"]').val(response.data.sport_organization_year_start).trigger('change'),
      $('[name="year_end"]').val(response.data.sport_organization_year_end).trigger('change'),
      $('#form-organization [name="name"]').val(response.data.sport_organization_name),
      $('[name="number_of_member"]').val(response.data.sport_organization_number_of_member),
      ...
      $('#modal-organization .modal-title').text('Ubah Informasi Organisasi Olahraga'),
...
function deleteMethod(redirect, scope, id) {
  Swal.fire({
    ...
    if(result.value) {
      $.ajax({
          url: $('meta[name=site-url]').attr("content")+'sport_organizations/delete?scope='+scope+'&id='+id,
          type: "DELETE",
          success: function(response){
            if (redirect == true) {
              location.reload();
