...
function initButton() {
  ...
  $('[key="add-club"]').on("click", function () {
    scope  = $(this).data("scope");
    value   = 'add';

    $('#modal-club .modal-title').text('Tambah Data Klub Olahraga'),
    $('#modal-club [type="submit"]').text('Tambah');
  });
...
function initModal() {
  $("#modal-club").on("hidden.bs.modal", function () {
    resetForm('club'),
    $('.select2').val('').trigger('change'),
    $('.dropify-clear').click(),
...
function fetchClub() {
  var dataTable = $('#dt_clubs').DataTable({
    oLanguage: {
    ...
    "order": [],
    "ajax": {
      url: $('meta[name=site-url]').attr("content")+"sport_clubs/list?scope="+$('meta[name=scope]').attr("content")+"&format=DT",
      type: "POST",
      processData: true,
    },
    "columnDefs": [{
  ...
  $("#dt_clubs_paginate").addClass("pagination-rounded");
}

$("#form-club").validate({
  submitHandler: function(form) {
    $.ajax({
      url:  $('meta[name=site-url]').attr("content")+'sport_clubs/store?scope='+scope+'&id='+value,
      data: new FormData($('#form-club')[0]),
      type: "POST",
      contentType: false,
      beforeSend: function() {
        requestBefore('modal');
      ...
      success: function(response) {
        setTimeout(function() {
          pushToastr(response.type, response.header, response.message.success), requestSuccess('modal'), $('#modal-club').modal('hide');
        }, 2e3)
      },
      error: function() {
        setTimeout(function() {
...
function putClub(method, id) {
  scope = method;
  value  = id;

  $.ajax({
    url:  $('meta[name=site-url]').attr("content")+'sport_clubs/get',
    data: "scope="+scope+"&format=JSON&id="+id,
    type: "GET",
    success: function(response) {
      $('[name="code"]').val(response.data.sport_club_code),
      $('[name="year"]').val(response.data.sport_club_year).trigger('change'),
      $('#form-club [name="name"]').val(response.data.sport_club_name),
      $('[name="leader"]').val(response.data.sport_club_leader),
      $('[name="address"]').val(response.data.sport_club_address),
      $('[name="province"]').val(response.data.province_id).trigger('change');
      $('[name="sport_organization_id"]').val(response.data.sport_organization_id).trigger('change');

      if (response.data.sport_club_file_id) {
      ...
      $('#modal-club .modal-title').text('Ubah Informasi Klub Olahraga'),
      $('#modal-club [type="submit"]').text('Simpan');
    },
  });
}
...
function deleteMethod(redirect, scope, id) {
  Swal.fire({
    ...
    if(result.value) {
      $.ajax({
          url: $('meta[name=site-url]').attr("content")+'sport_clubs/delete?scope='+scope+'&id='+id,
          type: "DELETE",
          success: function(response){
            if (redirect == true) {
              location.reload();
