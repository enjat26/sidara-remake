...
function initButton() {
  ...
  $('[key="export-print"]').on("click", function () {
    $('#form-export').attr('action', $('meta[name=site-url]').attr("content")+'sport_assets/export/print'),
    $('#form-export').submit();
  });

  $('[key="export-pdf"]').on("click", function () {
    $('#form-export').attr('action', $('meta[name=site-url]').attr("content")+'sport_assets/export/pdf'),
    $('#form-export').submit();
  });

  $('[key="export-excel"]').on("click", function () {
    $('#form-export').attr('action', $('meta[name=site-url]').attr("content")+'sport_assets/export/excel'),
    $('#form-export').submit();
  });
...
function chartAsset() {
  $.ajax({
    url:  $('meta[name=site-url]').attr("content")+'sport_assets/show',
...
function fetchAsset() {
  var dataTable = $('#dt_assets').DataTable({
    ...
    "order": [],
    "ajax": {
      url: $('meta[name=site-url]').attr("content")+"sport_assets/list?scope="+$('meta[name=scope]').attr("content")+"&format=DT",
      type: "POST",
      processData: true,
    },
    "columnDefs": [{
...
$("#form-category").validate({
  submitHandler: function(form) {
    $.ajax({
      url:  $('meta[name=site-url]').attr("content")+'sport_assets/store?scope='+$("#modal-category").data("scope")+'&id='+value,
...
$("#form-asset").validate({
  submitHandler: function(form) {
    $.ajax({
      url:  $('meta[name=site-url]').attr("content")+'sport_assets/store?scope='+scope+'&id='+value,
...
function putCategory(method, id) {
  ...
  $.ajax({
    url:  $('meta[name=site-url]').attr("content")+'sport_assets/get',
...
function deleteMethod(redirect, scope, id) {
  Swal.fire({
    ...
    if(result.value) {
      $.ajax({
          url: $('meta[name=site-url]').attr("content")+'sport_assets/delete?scope='+scope+'&id='+id,
          type: "DELETE",
          success: function(response){
            if (redirect == true) {
              location.reload();
