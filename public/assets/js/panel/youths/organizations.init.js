...
function submitExport(param){
  $('[key="export-'+param+'"]').on("click", function () {
    $("#form-export").attr("action", $("meta[name=site-url]").attr("content")+"youth_organizations/export/"+param);
    $("#form-export").submit();
  });
}
...
function initButton() {
  ...
  $('[key="rfs-organization"]').on("click", function () {
    $(".select2").val("").trigger("change"), initializeData();
  });
...
function fetchOrganization() {
  var dataTable = $("#dt_organizations").DataTable({
    ...
    ajax: {
      url:
        $("meta[name=site-url]").attr("content") +
        "youth_organizations/list?scope=" +
...
$("#form-organization").validate({
  submitHandler: function (form) {
    $.ajax({
      url:
        $("meta[name=site-url]").attr("content") +
        "youth_organizations/store?scope=" +
        scope +
        "&id=" +
        value,
  ...
  $.ajax({
    ...
    success: function (response) {
      $('[name="code"]').val(response.data.youth_organization_code),
        $('[name="year_start"]')
          .val(response.data.youth_organization_year_start)
          .trigger("change"),
        $('[name="year_end"]')
          .val(response.data.youth_organization_year_end)
          .trigger("change"),
        $('#form-organization [name="name"]').val(
          response.data.youth_organization_name
        ),
        $('[name="leader"]').val(response.data.youth_organization_leader),
        $('[name="number_of_member"]').val(
          response.data.youth_organization_number_of_member
        ),
      ...
      $("#modal-organization .modal-title").text(
        "Ubah Informasi Organisasi Pemuda"
      ),
  ...
  Swal.fire({
    ...
    if (result.value) {
      $.ajax({
        url:
          $("meta[name=site-url]").attr("content") +
          "youth_organizations/delete?scope=" +
          scope +
          "&id=" +
          id,
        type: "DELETE",
        success: function (response) {
          if (redirect == true) {
            ...
            pushSwal(response.type, response.header, response.message.success),
              initializeData();
          }
        },
      });
